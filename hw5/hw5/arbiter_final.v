...
module arbiter ( clock, reset, R0, R1, G0, G1 );
  ...
  DFFR_X1 current_state_reg_4_ ( .D(next_state[4]), .CK(clock), .RN(reset), 
        .Q(current_state[4]) );
  DFFR_X1 current_state_reg_2_ ( .D(next_state[2]), .CK(clock), .RN(reset), 
        .Q(current_state[2]) );
  DFFR_X1 current_state_reg_1_ ( .D(next_state[1]), .CK(clock), .RN(n17), .Q(
        current_state[1]), .QN(n33) );
  DFFR_X1 current_state_reg_3_ ( .D(next_state[3]), .CK(clock), .RN(n17), .Q(
        current_state[3]), .QN(n31) );
  DFFS_X1 current_state_reg_0_ ( .D(next_state[0]), .CK(clock), .SN(n17), .Q(
        current_state[0]), .QN(n32) );
